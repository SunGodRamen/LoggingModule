...
void init_logger(char* filePath) {
    ...
    time(&rawtime);
    localtime_s(&timeinfo, &rawtime); // Use localtime_s instead of localtime

    strftime(buffer, sizeof(buffer), "%Y-%m-%d %H:%M:%S", &timeinfo); // Notice the & for timeinfo

    // Print the run date to the log file
    fprintf(logFile, "---- Run date: %s ----\n", buffer);
...
static void write_to_log_file(LogLevel level, const char* message) {
    ...
    if (fprintf(logFile, "%s %s\n", levelStr, message) < 0) {
        printf(stderr, "Error: Unable to write to log file.\n");
    }
